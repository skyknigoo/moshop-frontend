...
import api from '@/api/axios'; // ✅ 使用你寫好的攔截器實例
...
const fetchProfile = async () => {
    try {
        const res = await api.get('/AccountApi/Profile')
        user.value = res;
    } catch (e) {
...
const onFormSubmit = async ({ valid, values }) => {
    if (valid) {
        ...
        try {
            ...
            toast.add({ severity: 'error', summaty: '密碼修改失敗', detail: errorMsg, life: 5000 });
            initialValues.value = { OldPassword: '', NewPassword: '', ConfirmPassword: '' }
        ...
        <Card style=" width: 30rem; overflow: hidden;padding: 10px;">
            ...
            <template #content>
                ...
                <div class="text-center py-2">
                    <PButton icon="pi pi-shield" label="修改登入密碼" severity="success" outlined
                ...
                <div v-if="showPasswordSection" class="p-4 surface-50 border-round-lg border-1 surface-border">
                    ...
                    <PForm v-slot="$form" :resolver="resolver" :initialValues="initialValues" @submit="onFormSubmit"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <Message class="text-red-400 -m-1" v-if="$form?.OldPassword?.invalid" severity="error"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <Message class="text-red-400 -m-1" v-if="$form?.NewPassword?.invalid" severity="error"
                                size="small" variant="simple">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <Password name="ConfirmPassword" id="ConfirmPassword"
                                v-model="initialValues.ConfirmPassword" fluid />
                            <Message class="text-red-400 -m-1" v-if="$form?.ConfirmPassword?.invalid" severity="error"
                                size="small" variant="simple">
                        ...
                        <div v-if="showPasswordSection" class="flex gap-3 mt-2">
                            ...
                            <PButton label="取消修改" severity="secondary" outlined class="flex-1 py-3"
                                @click="togglePasswordFields" />
                        </div>
                    </PForm>
