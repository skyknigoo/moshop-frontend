<template>
  <div class="p-4 surface-ground min-h-screen">
    ...
    <div class="flex justify-content-between align-items-center mb-4">
      ...
      <PButton label="新增商品" icon="pi pi-plus" class="p-button-lg shadow-2" @click="router.push('/manage/product/create')" />
    </div>
      ...
      <template #content>
        <div class="grid p-fluid">
          ...
          <div class="col-12 lg:col-2">
            ...
            <Select v-model="filters.groupId" :options="categories" optionLabel="groupName" optionValue="groupID" placeholder="全部類別" />
          </div>
    ...
    <div class="surface-card border-round shadow-1">
      <DataTable :value="products" :loading="loading" responsiveLayout="scroll" :rows="10" 
                 class="p-datatable-sm" hoverSelection>
        <Column header="商品縮圖" class="pl-4">
          <template #body="{ data }">
            <PImage :src="data.imagePath || '/uploads/Comm/等待餵圖.png'" width="60" class="shadow-1 border-round" preview />
          </template>
        </Column>
        <Column header="商品名稱">
          <template #body="{ data }">
  ...
  <template #body="{ data }">
    <div class="flex gap-2 justify-content-end">
      ...
      <PButton 
        :label="data.status === 2 ? '解凍' : '凍結'" 
        :icon="data.status === 2 ? 'pi pi-unlock' : 'pi pi-lock'" 
        :severity="data.status === 2 ? 'warn' : 'info'" 
        outlined 
        size="small" 
        @click="confirmFrozen(data)" 
      />
...
const loadData = async () => {
  ...
  try {
    // 呼叫後端 API，注意路徑需與後端對齊
    const res = await axios.get('http://localhost:5158/api/manage/MProductApi', { params: filters.value });
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        ...
        const res = await axios.post(`http://localhost:5158/api/manage/MProductApi/Frozen/${product.productID}`);
        if (res.data.success) {
...
const confirmDelete = (product) => {
  confirm.require({
    ...
    accept: async () => {
      try {
        // 修正路徑：api/manage/MProductApi
        const res = await axios.post(`http://localhost:5158/api/manage/MProductApi/Delete/${product.productID}`);
        if (res.data.success) {
